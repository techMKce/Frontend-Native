  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fetchedSubmission = subData.submission;

        const gradingRes = await fetch(
          `https://assignmentservice-2a8o.onrender.com/api/gradings?assignmentId=${fetchedSubmission.assignmentId}`
        );
        const gradingData = await gradingRes.json();

        if (!gradingRes.ok || !gradingData.gradings) {
