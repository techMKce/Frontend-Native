...
//     file: null as { name: string; type: string; uri: string; base64?: string } | null,
//     link: '',
//     removeFile: false,
//   });
...
//         file: assignment.file ? { name: 'Existing File', type: 'unknown', uri: assignment.file } : null,
...
//       await axios.delete('https://assignmentservice-2a8o.onrender.com/api/assignments', {
//         params: { assignmentId },
//         timeout: 10000,
//       });
...
//       return;
//     }
//     if (formData.link && !/^(https?:\/\/)?([\w-]+\.)+[\w-]+(\/[\w-./?%&=]*)?$/.test(formData.link)) {
...
//       if (formData.dueDate) formDataPayload.append('dueDate', formatDateTime(formData.dueDate));
//       formDataPayload.append('resourceLink', formData.link || '');
//       if (formData.file && formData.file.base64) {
//         const blob = new Blob([Buffer.from(formData.file.base64.split(',')[1], 'base64')], { type: formData.file.type });
//         formDataPayload.append('file', blob, formData.file.name);
//       } else {
//         formDataPayload.append('file', new Blob([]), 'empty.txt');
//       }

//       await axios.post('https://assignmentservice-2a8o.onrender.com/api/assignments', formDataPayload, {
...
//     assignmentId: string,
//     formData: { courseId: string; title: string; description: string; dueDate: Date | null; file: any; link: string; removeFile: boolean }
//   ) => {
//     if (!formData.title.trim()) {
//       Alert.alert('Error', 'Title cannot be empty.');
//       return;
...
//       if (formData.courseId) formDataPayload.append('courseId', formData.courseId);
...
//       await axios.put('https://assignmentservice-2a8o.onrender.com/api/assignments', formDataPayload, {
//         headers: { 'Content-Type': 'multipart/form-data' },
//         timeout: 10000,
//       });
//       setAssignments((prev) =>
...
//                 dueDate: formData.dueDate ? formatDateTime(formData.dueDate) : assignment.dueDate,
//                 file: formData.removeFile ? null : formData.file?.uri || assignment.file,
//                 link: formData.link,
//               }
//             : assignment
//         )
...
//     setModalType(null);
//     setShowDatePicker(false);
//     setForm({ courseId: 'default-course', title: '', description: '', dueDate: null, file: null, link: '', removeFile: false });
//   };
...
//                     value={form.courseId}
//                     onChangeText={(text) => setForm({ ...form, courseId: text })}
...
//   modalContent: { backgroundColor: COLORS.white, borderRadius: 12, padding: SPACING.lg, width: '80%', maxWidth: 400, ...SHADOWS.medium },
...
//   fileButton: { backgroundColor: COLORS.secondary, borderRadius: 8, paddingVertical: SPACING.sm, paddingHorizontal: SPACING.md },
//   fileButtonText: { fontFamily: FONT.medium, fontSize: SIZES.sm, color: COLORS.white },
//   filePreview: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.lightGray, borderRadius: 8, padding: SPACING.sm, flex: 1 },
//   fileName: { fontFamily: FONT.regular, fontSize: SIZES.sm, color: COLORS.darkGray, flex: 1 },
...
export default function AssignmentsScreen() {
  ...
  const performDelete = async (assignmentId: string) => {
    try {
      setDeletingId(assignmentId);
      const response = await axios.delete('https://assignmentservice-2a8o.onrender.com/api/assignments', {
        params: { assignmentId },
        timeout: 10000,
      });
      setAssignments((prev) => prev.filter((assignment) => assignment.assignmentId !== assignmentId));
  ...
  const performCreate = async () => {
    ...
    try {
      ...
      if (form.dueDate) {
        ...
        formDataPayload.append('dueDate', ''); // Backend expects a string; empty if no due date
      }
      formDataPayload.append('resourceLink', form.link || '');

      // Backend expects a single 'file' field, so we'll use the first file from the files array
      if (files.length > 0) {
        const file = files[0]; // Take the first file since backend expects a single file
        ...
        // Backend requires a file; if none is provided, validation should catch it
        throw new Error('No file selected');
      }

      const response = await axios.post('https://assignmentservice-2a8o.onrender.com/api/assignments', formDataPayload, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 10000,
      });
  ...
  const performEdit = async (assignmentId: string) => {
    ...
    try {
      ...
      // Backend expects a single 'file' field for updates
      if (files.length > 0) {
        const file = files[0]; // Take the first file since backend expects a single file
        formDataPayload.append('file', {
          uri: file.uri,
      ...
      const response = await axios.put('https://assignmentservice-2a8o.onrender.com/api/assignments', formDataPayload, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 10000,
      });
        ...
        prev.map((assignment) =>
          assignment.assignmentId === assignmentId
            ? {
                ...
                file: files.length > 0 ? files[0].uri : null,
                link: form.link,
              }
            : assignment
        )
