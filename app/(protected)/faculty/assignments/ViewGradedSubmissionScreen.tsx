...
const ViewGradedSubmissionScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch submission data using axios
        const subRes = await api.get(`/submissions/id?submissionId=${submissionId}`);
        const fetchedSubmission: Submission = subRes.data.submission;

        if (!fetchedSubmission) {
          throw new Error('Submission not found');
        }

        // Fetch grading data using axios
        const gradingRes = await api.get(
          `/gradings?assignmentId=${fetchedSubmission.assignmentId}`
        );
        const gradingData: GradingResponse = gradingRes.data;

        if (!Array.isArray(gradingData.gradings)) {
