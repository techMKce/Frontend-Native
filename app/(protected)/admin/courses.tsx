...
export default function CoursesManagementScreen() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to load courses. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error) {
    return (
      <View style={styles.container}>
        <Header title="Courses Management" />
        <View style={[styles.content, { justifyContent: 'center', alignItems: 'center' }]}>
          <Text style={{ color: COLORS.error, fontFamily: FONT.medium, fontSize: SIZES.md }}>
            {error}
          </Text>
          <TouchableOpacity 
